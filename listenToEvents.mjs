...
import { decodeServiceEnvelope, processDecodedPacket } from './protobufDecoder.mjs'
...
const handleProtobufServiceEnvelopePacket = async (server, fullTopic, user, arrayBuffer, callback) => {
  try {
    ...
    client.on('message', async (topic, payload, packet) => {
      try {
        ...
        if (payload && payload.length > 0) {
          await handleProtobufServiceEnvelopePacket(server, topic, user, new Uint8Array(payload), callback)
        }
      } catch (error) {
