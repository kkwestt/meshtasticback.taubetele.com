...
export class MQTTManager {
  ...
  async connectToServer(server) {
    return new Promise((resolve, reject) => {
      ...
      // Используем кастомные топики сервера или дефолтные
      const topics = server.topics || this.getTopicList();
      console.log(
        `📋 [MQTT-Receiver] [${server.name}] Топики для подписки: ${topics.join(", ")}`
      );
